...
const CreateQuotePage: React.FC<CreateQuotePageProps> = ({ currentUser: propCurrentUser }) => {
  ...
  if (!companyInfo && !localStorage.getItem('companyInfo') && !isEditMode) { 
    return (
      <div className="p-6 bg-gray-800 shadow-xl rounded-lg text-center text-gray-300">
        ...
        <p className="text-gray-400">Por favor, configure as informações da sua empresa na seção "Empresa" antes de criar orçamentos.</p>
   ...
   if (!loggedInUser) {
    return (
      <div className="p-6 bg-gray-800 shadow-xl rounded-lg text-center text-gray-300">
        ...
        <p className="text-gray-400">Não foi possível identificar o usuário. Por favor, tente fazer login novamente.</p>
  ...
  return (
    <div className="p-6 bg-gray-800 shadow-xl rounded-lg text-gray-300">
      ...
      {selectedCustomerId && clientName && ( // Display details only if selectedCustomerId and clientName (derived) are present
          <div className="mb-4 p-3 bg-gray-700 border border-gray-600 rounded-md text-sm text-gray-200">
      ...
      {loggedInUser && (
        <p className="text-sm text-gray-400 mb-6">
          Vendedor Responsável: <span className="font-medium text-gray-200">{loggedInUser.fullName || loggedInUser.username}</span>
        </p>
      )}
      
      <div className="mb-6 p-4 border border-gray-700 rounded-md">
        <h3 className="text-lg font-medium text-gray-200 mb-3">Adicionar Itens ao Orçamento</h3>
          ...
          {selectedProductDetails?.pricingModel === PricingModel.PER_SQUARE_METER && (
            <>
              ...
              {itemWidth > 0 && itemHeight > 0 && itemCountForArea > 0 &&
                <p className="text-sm text-gray-400 md:mt-6">
      ...
      {quoteItems.length > 0 && (
        <div className="mb-6">
          <h3 className="text-lg font-medium text-gray-200 mb-3">Itens do Orçamento</h3>
            ...
            <table className="min-w-full divide-y divide-gray-700">
              ...
              <tbody className="bg-gray-800 divide-y divide-gray-700">
                {quoteItems.map((item, index) => {
                  ...
                  return (
                    <tr key={index} className="hover:bg-gray-700/60">
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-200">{item.productName}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-300">{qtyDisplay}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-300">{formatCurrency(item.unitPrice)}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-300">{formatCurrency(item.totalPrice)}</td>
       ...
       <div className="mb-6 p-4 border border-gray-700 rounded-md">
        <h3 className="text-lg font-medium text-gray-200 mb-3">Descontos</h3>
      ...
      <div className="mb-6 p-4 border border-gray-700 rounded-md">
        <h3 className="text-lg font-medium text-gray-200 mb-3">Informações de Pagamento e Entrega</h3>
      ...
      {quoteItems.length > 0 && (
        <div className="mt-6 p-6 bg-gray-850 rounded-lg shadow-xl">
          <h3 className="text-xl font-semibold text-white mb-4 text-center">Resumo do Orçamento</h3>
          <div className="space-y-2 text-sm mb-4">
            <div className="flex justify-between">
              <span className="text-gray-400">Subtotal:</span>
              <span className="font-medium text-gray-200">{formatCurrency(subtotal)}</span>
            </div>
            {discountAmountCalculated > 0 && (
              <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-center mb-3">
            <div>
              <p className="text-md font-medium text-gray-300">TOTAL (À VISTA - sem sinal)</p>
              <p className="text-2xl font-bold text-yellow-500">{formatCurrency(totalCash)}</p>
            </div>
            <div>
              <p className="text-md font-medium text-gray-300">TOTAL (CARTÃO - sem sinal)</p>
